...
const FindYourOwnStyle = ({
  ...
  let showFilter = Boolean(
    new URLSearchParams(location.search).get("showFilter")
  );
  ...
  useEffect(() => {
    ...
    if (showFilter) {
      ...
      } else if (user && (user.bodyType != null || user.bodyType != "")) {
        setShowModal(true);
      } else {
        setShowModal(true);
              ...
              <div class="modal-body">
                {result != "" ? (
                  <>
                    ...
                    >
                      {user == null
                        ? "Login to Save Body Type and Filter Dresses"
                        : "Save Body Type"}
                    </button>
                  </>
                ) : (
...
export default connect(mapStateToProps, { setFilters, setBodyType })(
  FindYourOwnStyle
);
